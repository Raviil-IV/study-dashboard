...
Все таблицы данных пользователя — с внешним ключом `user_id` → `users.id`, каскадное удаление при удалении аккаунта. Идентификаторы записей — UUID, генерирует сервер.
...
| `POST` | `/api/lessons` `/api/tasks` `/api/deadlines` `/api/notes` `/api/focus-sessions` | создать запись → возвращает созданную (с серверным `id`) |
