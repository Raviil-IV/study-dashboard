...
Статус: принята. Дельта от 2026-08-09: авторизация по логину (без email), пароль мин. 6 символов, отображение логина и кнопки «Выйти» в интерфейсе.
...
- **Регистрация открытая**: любой желающий может создать аккаунт (логин + пароль).
...
| Таблица | Колонки |
|---|---|
| `users` | `id uuid PK`, `login text unique not null`, `password_hash text not null`, `created_at timestamptz` |
...
|---|---|---|
| `POST` | `/api/auth/register` | `{login, password}` → создаёт пользователя, ставит куку |
| `POST` | `/api/auth/login` | `{login, password}` → ставит куку |
| `POST` | `/api/auth/logout` | выход, чистит куку |
| `GET` | `/api/auth/me` | текущий пользователь (`{id, login}`) — проверка сессии |
...
Примечание: `POST /api/auth/register` при занятом логине — `409` («Логин уже занят»).

## 7. Авторизация и безопасность

- Логин: 3–32 символа, только латинские буквы, цифры, `_` и `-` (проверяется на клиенте и на сервере через zod).
- Пароль: минимум 6 символов, проверяется на клиенте и на сервере (zod).
...
- **Новые страницы**: `/login` и `/register` — формы логин + пароль, инлайн-ошибки от сервера, редирект на главную после успеха.
- **Защита маршрутов**: `ProtectedRoute` — при загрузке приложение вызывает `GET /api/auth/me`; `401` → редирект на `/login`.
- **Отображение пользователя**: логин показывается внизу сайдбара (десктоп) и в мобильной шапке, рядом — кнопка «Выйти» (выход чистит куку, сбрасывает стор и возвращает на `/login`).
...
3. Регистрация (логин + пароль), вход, выход работают; при занятом логине — понятная ошибка.
